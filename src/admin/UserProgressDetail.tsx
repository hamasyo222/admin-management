...
import { LearningProgressChart } from '../dashboard/LearningProgressChart';
import { ContentCompletionChart } from '../dashboard/ContentCompletionChart';
import { LearningTimeDistribution } from '../dashboard/LearningTimeDistribution';
...
export const UserProgressDetail: React.FC<UserProgressDetailProps> = ({
  ...
  const totalStudyTime = userProgress.reduce((total, p) => total + p.timeSpent, 0);
  ...
  useEffect(() => {
    // Prepare chart data when component mounts or userProgress changes
    prepareChartData();
  }, [userProgress]);

  const prepareChartData = () => {
    // This function would prepare data for any additional charts
    // that aren't already handled by the chart components
  };
  ...
  const handleGenerateReport = () => {
    try {
      ...
      doc.setFontSize(12);
      doc.text(`ユーザー: ${user.lastName} ${user.firstName}`, 20, 40);
      doc.text(`メール: ${user.email}`, 20, 50);
      doc.text(`ユーザー種別: ${user.userType}`, 20, 60);
      ...
      // Save the PDF
      doc.save(`学習進捗レポート_${user.lastName}${user.firstName}.pdf`);
        ...
        <div className="flex items-center">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-900">
              {user.lastName} {user.firstName}の学習進捗
            </h2>
            <p className="text-sm text-gray-500">
              {user.email} • {user.userType === 'company_admin' ? '企業管理者' : 
                            user.userType === 'company_employee' ? '企業従業員' : 
                            user.userType === 'dx_talent' ? 'DX人材' : 
                            user.userType === 'foreign_talent' ? '海外人材' : 
                            user.userType === 'admin' ? 'システム管理者' : 
                            'サポートスタッフ'}
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="h-64">
              <LearningProgressChart userProgress={userProgress} />
            </div>
          </div>
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="h-64">
              <LearningTimeDistribution userProgress={userProgress} />
            </div>
          </div>
